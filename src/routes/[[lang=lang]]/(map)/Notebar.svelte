<script lang="ts">
    ...
    const notebarSize = writable<{ start: number, end: number, length: number }>({start: 0, end: 0, length: 0});
    const gimmickMarkers = writable<{
        [k: number]: { layerGlobal: number, layerLocal: number, relativeGimmickLength: number | undefined }
    }>({});
    const gimmickHighlightsByGimmickId = writable<{ [k: number]: Set<number> }>({});
    const gimmickHighlightsByNoteId = writable<{ [k: number]: Set<number> }>({});
    ...
    $: {
        ...
        gimmickMarkerTrackers = {global: new MarkerTracker()};
