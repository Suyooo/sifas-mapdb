...
function freeLiveCollapsibleOpen() {
    ...
    if (tabs.$activeTabLink.attr("href").endsWith("story")) {
        ...
        pagename = $(".song-name.translatable", this.el).data(showRomaji ? "rom" : "jpn") + " (Story " + $(".active", "#" + tabs.$activeTabLink.attr("href").substring(1)).text().split(" (")[0].trim() + ")";
    } else {
        location = "live" + tabs.$activeTabLink.attr("href").substring(1);
        pagename = $(".song-name.translatable", this.el).data(showRomaji ? "rom" : "jpn") + " (" + tabs.$activeTabLink.text() + ")";
    }
...
function freeLiveTabShow(tabs, e) {
    if ($(e).hasClass("live-difficulty")) {
        ...
        addHistoryItem("live" + $(e).attr("id"), $(".song-name.translatable", $(e).parent().parent()).data(showRomaji ? "rom" : "jpn") + " (" + tabs.$activeTabLink[0].childNodes[0].data.trim() + ")");
    } else {
        // Story Stages tab
        let tabElement = $(".tabs", e)[0];
        let tabs = M.Tabs.getInstance(tabElement);
        addHistoryItem("live" + tabs.$activeTabLink.attr("href").substring(1), $(".song-name.translatable", $(e).parent().parent()).data(showRomaji ? "rom" : "jpn") + " (Story " + tabs.$activeTabLink[0].childNodes[0].data.split(" (")[0].trim() + ")");
...
function freeLiveStoryTabShow(tabs, e) {
    ...
    scrollActiveTabLabelIntoView(tabs);
    addHistoryItem("live" + $(e).attr("id"), $(".song-name.translatable", $(e).parent().parent().parent()).data(showRomaji ? "rom" : "jpn") + " (Story " + tabs.$activeTabLink[0].childNodes[0].data.split(" (")[0].trim() + ")");
}
...
function dlpTowerCollapsibleOpen() {
    ...
    addHistoryItem(towerLink, $($(".song-name.translatable", this.$el)[0]).data(showRomaji ? "rom" : "jpn").replaceAll("Dream Live Parade", "DLP").replaceAll("ドリームライブパレード", "DLP"));
}

function dlpFloorCollapsibleInit() {
    let towerLink = "tower" + $(this).parent().parent().parent().attr("id");
    let towerPageName = $($(".song-name.translatable", $(this).parent().parent().parent())[0]).data(showRomaji ? "rom" : "jpn").replaceAll("Dream Live Parade", "DLP").replaceAll("ドリームライブパレード", "DLP");
    let collapsible = M.Collapsible.getInstance(this);
...
function dlpFloorCollapsibleOpen(towerPageName) {
    ...
    addHistoryItem("floor" + $(this).attr("id"), towerPageName + " ➔ " + $(".song-name.translatable", $(this)).data(showRomaji ? "rom" : "jpn"));
